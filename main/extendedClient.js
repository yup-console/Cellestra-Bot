...
module.exports = class ExtendedClient extends Client {
  constructor() {
    ...
    this.dab = new Database("mongodb+srv://z3kr2:wf0a3p@cluster0.x8fn2s8.mongodb.net/?retryWrites=true&w=majority");
    this.dab.connect()
      mongoose.connect("mongodb+srv://z3kr2:wf0a3p@cluster0.x8fn2s8.mongodb.net/?retryWrites=true&w=majority", {
    ...
    this.once("ready", async (client) => {
      ...
      require("@utils/playerMonitor.js")(this);
    });
  ...
  getPlayer = async (id) => {
    try {
      ...
      this.log(`Error getting player: ${error}`, "warn");
      return null;
    }
  };

  createPlayer = async (options) => {
    try {
      // Clean up any existing player first
      const existingPlayer = await this.getPlayer(options.guildId);
      if (existingPlayer) {
        await existingPlayer.destroy();
        await this.sleep(1000);
      }
      
      return await this.manager.createPlayer(options);
    } catch (error) {
      this.log(`Error creating player: ${error}`, "warn");
      throw error;
    }
  };
